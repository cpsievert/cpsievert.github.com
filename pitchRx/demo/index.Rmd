...
```{r ani, fig.show="animate", interval=0.01, cache=TRUE, dev="CairoPNG"}
...
```{r ani2, fig.show="animate", interval=0.01, cache=TRUE, fig.height=14, fig.width=10, dev="CairoPNG"}
...
```{r rglhook, echo=FALSE}
require(devtools)
library(knitr)
knit_hooks$set(webgl = hook_webgl)
```

```{r webgl1, webgl=TRUE, echo=FALSE, fig.width=18, fig.height=10}
#ugly workaround since knitr won't recognize interactiveFX
Rivera <- subset(pitches, pitcher_name=="Mariano Rivera")
install_github('pitchRx', 'cpsievert') #didn't export getSnapshots with v0.2
snaps <- getSnapshots(Rivera, interval = 0.05)
types <- as.character(Rivera[,"pitch_type"])
ncolors <- length(unique(types))
pal <- c(rgb(0, 0, 1), rgb(1, 0, 0))
...
```{r strike, fig.width=10, dev="CairoPNG"}
...
```{r strike2, fig.width=10, dev="CairoPNG"}
...
```{r strike4, fig.height=14, fig.width=10, dev="CairoPNG"}
...
```{r strike5, fig.height=14, fig.width=10, dev="CairoPNG"}
