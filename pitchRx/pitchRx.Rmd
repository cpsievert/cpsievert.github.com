...
```{r getData, results='hide', warning=FALSE, message=FALSE, cache=TRUE}
...
example2 <- join(pitches, data$game, by = "url", type = "inner") 
#Now create a faceting variable used for strikezone analysis
example2$home <- NULL
example2$home[example2$home_team_name == "Yankees"] <- "home"
example2$home[example2$home_team_name != "Yankees"] <- "away"
strikes <- example2[example2$des %in% c("Called Strike", "Ball"),]
strikeFX(strikes, aes(color=des))
```

## Faceting by batter stance

```{r stand, echo=FALSE}
strikeFX(strikes, layer=list(aes(color=des), facet_grid(.~stand)))
```

## Faceting by stance and "home"

```{r home, echo=FALSE}
strikeFX(strikes, layer=list(aes(color=des), facet_grid(top_inning~stand)))
```
...
strikeFX(data2, geom="hex", layer=facet_grid(top_inning~stand))
```

## HEX2

```{r hex2, echo=FALSE, cache=TRUE}
strikeFX(data2, geom="hex", layer=facet_grid(des~top_inning~stand))
```
...
```{r animate1, echo=FALSE, fig.show='animate'}
animateFX(pitches)
```

## And flexible!

```{r animate3, echo=FALSE, fig.show='animate'}
