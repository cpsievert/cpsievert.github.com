...
* The `object.size` for this game is 
* Thousands of games are played per year, so we surely don't want to bring all this data at once into an `R` session.
...
* `scrape` can export data to a database connection in a "chunks".
...
* If you know how to run `R CMD BATCH` jobs...set it and forget it!
