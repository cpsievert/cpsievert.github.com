...
% Carson Sievert `r library(lubridate); paste(month(Sys.Date()), day(Sys.Date()), year(Sys.Date()), sep = "/")`
...
opts_chunk$set(fig.path="figure/", cache.path="cache/", fig.align='left', warning=FALSE, message=FALSE, fig.height=10, fig.width=12, fig.cap='', tidy=FALSE, eval=FALSE)
...
```{r ex1, results='hide', cache=TRUE, eval=TRUE}
...
```{r ani, fig.show="animate", interval=0.01, cache=TRUE, dev="CairoPNG", echo=FALSE, eval=TRUE}
...
```{r ani-norm, fig.show="animate", interval=0.05, cache=TRUE, dev="CairoPNG", echo=FALSE, eval=TRUE}
animateFX(Darvish, avg.by="pitch_types", 
          layer=list(coord_equal(),
          theme_bw(),
          facet_grid(.~stand)))
