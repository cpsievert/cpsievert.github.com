...
Now that we have a bit of intuition, let's describe the model mathematically. For a given document $d$, the probability of observing word $i$ (in a fixed vocabulary $i \in \{1, \dots, V\}$) can be represented as

$$
p(w_i) = \sum_{j=1}^T p(w_i, z_j) = \sum_{j=1}^T p(w_i|z_j)p(z_j)
$$

where $z_j$ is the $j^{th}$ topic and $T$ is the total number of topics. The two main probabilities of interest here are $\phi_i^{(z_j)} = p(w_i|z_j)$ and $\theta_j^{(d)} = p(z_j)$. It can help to think of these quantities as matrices. The term-topic matrix $\Phi$ is $V$ by $T$ - where the $j^{th}$ column contains the probability distribution over words for topic $j$. The topic-document matrix $\Theta$ is $T$ by $D$ - where the $d^{th}$ column contains the topic probabilities for document $d$.
