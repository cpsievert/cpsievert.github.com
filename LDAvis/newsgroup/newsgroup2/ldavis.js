var LDAvis = function (to_select, json_file) {
...
// This section sets up the logic for event handling
    ...
    function change_topic(to_select) {

        var topicSel  = to_select + "-topic";
        var tSel = topicSel.replace("#", "");

        var new_topic = Math.max(1, Math.min(50, Math.floor(+document.getElementById("topic").value)));
        if (isNaN(new_topic)) {
    	current_clicked = { what: "nothing", element: undefined, object: undefined };
    	document.getElementById(tSel).value = current_topic;
        } else {
    	current_topic = new_topic;
    	current_clicked.element = d3.select(topicSel.concat(current_topic))[0][0];
    	current_clicked.what = "topic";
    	current_clicked.object = d3.select(topicSel.concat(current_topic))[0][0].__data__;
    	document.getElementById(tSel).value = current_topic;
    	update_drawing();
        }
        return false;
    ...
    // function to click a topic based on input from the html form:
    function decrement_topic(to_select) {
    ...
    // function to click a topic based on input from the html form:
    function increment_topic(to_select) {
    ...
    function change_lambda(to_select) {

        var lambdaSel  = to_select + "-lambda";
        var lambdaSel = lambdaSel.replace("#", "");

        // read in the new value of lambda from the html input:
        var new_lambda = Math.max(0, Math.min(100, Math.floor(+document.getElementById(lambdaSel).value * 100)))/100;
        
        // if the new lambda is not a number, just print the old lambda to the screen and do nothing else:
        if (isNaN(new_lambda)) {  
    	document.getElementById(lambdaSel).value = current_lambda;
    	//return false;
        } else {
    ...
    // function to read in a new lambda value from an html form and transition barchart:
    function decrement_lambda(to_select) {
    ...
    // function to read in a new lambda value from an html form and transition barchart:
    function increment_lambda(to_select) {
    ...
    d3.json(json_file, function(error, data) {
        ...
        //Insert the forms in the svg element
        var topicentry = d3.select(to_select).append("div")
            .attr("style", "float: left; margin-left: 30px; padding-bottom: 10px; font: 14px sans-serif; background-color:white");

        var topicform = topicentry.append("form")
                        .attr("class", "inlineForm")
                        .attr("onSubmit", "return change_topic()")
                        .text("Selected Topic:");

        topicform.append("input")
            .attr("type", "text")
            .attr("id", "topic")
            .attr("size", "10");
        ...
        // Div that handles value of lambda forms
       var lambdaentry = d3.select(to_select).append("div")
            .attr("style", "float: left; margin-left: 30px; padding-bottom: 10px; font: 14px sans-serif; background-color:white");

        // Input to type value of lambda
        var lambdaform1 = lambdaentry.append("form")
            .attr("class", "inlineForm")
            .attr("onSubmit", "return change_lambda()")
            .text("Lambda = ");
        
        lambdaform1.append("input")
            .attr("type", "text")
            .attr("id", "lambda")
            .attr("size", "10");
        ...
        //Input buttons for increment/decrement of lambda
        var lambdaform2 = lambdaentry.append("form")
            .attr("class", "inlineForm")
            .attr("onSubmit", "return decrement_lambda()");

        lambdaform2.append("input")
            .attr("type", "submit")
            .attr("value", "&#955 - 0.1");

        var lambdaform3 = lambdaentry.append("form")
            .attr("class", "inlineForm")
            .attr("onSubmit", "return increment_lambda()");

        lambdaform3.append("input")
            .attr("type", "submit")
            .attr("value", "&#955 + 0.1");
        ...
        points.append("circle")
            ...
            .attr("id", function(d) { return (to_select + '-topic' + d.topics) })
